...
def test_it_falls_back_on_what_it_refuses(hooked):
    # A compiler setting this layer does not read, and the caller must
    # not be able to tell. Compared against a live ct3 rather than a
    # written-down string: what ct3 does with the whitespace around a
    # block comment once that gobbling is off is the question, and a
    # guess at it would test the guess.
